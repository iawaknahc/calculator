type t = { sign : float; has_dot : bool; appendable : bool; value : string }
...
let append_dot buf =
  if not buf.appendable then new_dot
  else if buf.has_dot then buf
  else { buf with has_dot = true; value = buf.value ^ "." }

let append_number buf max n =
  if n < 0 || n > 9 then buf
  else if not buf.appendable then new_number n
  else
    match buf.value with
    | "0" -> { buf with value = string_of_int n }
    | value ->
        let len = String.length value in
        let len = if buf.has_dot then len - 1 else len in
        if len >= max then buf
        else { buf with value = value ^ string_of_int n }
