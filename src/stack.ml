...
type item = S_Buffer of Buffer.t | S_Op of Op.t
...
let append_number stack ~max ~n =
  match stack with
  | S_Buffer b :: rest -> S_Buffer (Buffer.append_number b max n) :: rest
  | rest -> S_Buffer (Buffer.new_number n) :: rest |> simplify

let append_dot = function
  | S_Buffer b :: rest -> S_Buffer (Buffer.append_dot b) :: rest
  | rest -> S_Buffer Buffer.new_dot :: rest |> simplify

let append_op stack op =
  match stack with
  | [] -> [ S_Op op; S_Buffer (Buffer.new_number 0) ]
  | S_Op _ :: rest -> S_Op op :: rest
  | rest -> S_Op op :: rest |> simplify

let negate = function
  | S_Buffer b :: rest -> S_Buffer (Buffer.toggle_sign b) :: rest
  | rest -> S_Buffer Buffer.new_sign :: rest |> simplify

let cancel = function
  | S_Op _ :: _ as rest -> S_Buffer (Buffer.new_number 0) :: rest |> simplify
  | S_Buffer _ :: rest -> S_Buffer (Buffer.new_number 0) :: rest
  | rest -> rest

let top_display = function
  | [] -> "0"
  | S_Buffer b :: _ -> Buffer.format b
  ...
  | [ S_Op _; S_Buffer a ] -> Buffer.format a
  | _ -> failwith "unreachable"

let eval = function
  | [] -> 0.0
  | [ S_Buffer b ] -> Buffer.to_float b
  | [ S_Buffer c; S_Op op2; S_Buffer b; S_Op op1; S_Buffer a ] ->
      let a = Buffer.to_float a in
      let b = Buffer.to_float b in
      let c = Buffer.to_float c in
      if Op.(op1 >= op2) then Op.apply op2 (Op.apply op1 a b) c
...
let last_operation = function
  | S_Buffer b :: S_Op op :: _ -> Some (op, Buffer.to_float b)
  | _ -> None
...
let eq = function
  ...
  | S_Op _ :: _ as stack -> (
      ...
      with Failure _ -> ([ S_Buffer (Buffer.new_result nan) ], None) )
  | [] -> ([], None)
